...
int main(void)
{
	...
	for (int i = 0; i < 50; i++) {	// 500명 학번, 이름. 학점 생성
		...
		head = insert_first(head, num, name1, name2, name3, score);
		// 리스트 맨 앞에 노드 생성
	...
	for (ListNode* k = head->link; k != head; k = k->link) {
		if (k->score < a) {
			printf("%d\t%c%c%c\t%.2lf\n", k->num, k->name1, k->name2, k->name3, k->score);
			i++;
		}
	}
	printf("기준 점수보다 아래인 학생들은 총 %d 명 입니다.", i);
