...
void insert_last(ArrayListType *L, element item)
{
	...
	L->array[L->size++] = item;				// *데이터 삽입 후 size값 증가*
}
void insert(ArrayListType *L, int pos, element item)
{
	if (!is_full(L) && (pos >= 0) && (pos <= L->size)) {
		for (int i = (L->size - 1); i >= pos; i--)		// i는 -1(초기값) 부터 pos(0)보다 클때=> 동작 안함
			L->array[i + 1] = L->array[i];
		L->array[pos] = item;							// item(10)을 pos(0)에 넣음
		L->size++;										// size(0)에서 size(1)로 증가`
	}
}
