...
void queue_print(QueueType* q) {            // 불량품만 따로 저장하는 파일을 만드는 함수
    ...
    if (!is_empty(q)) {         // 큐 타입이 비어있지 않은 경우 프로그램을 실행함
        ...
        do {
            i = (i + 1) % (MAX_QUEUE_SIZE);         // i의 값은 i + 1 을 MAX(10000)으로 나눈 나머지 이므로 i+1값이 됨 (처음 한번만 실행)
...
int main() {
    ...
    for (int k = 0; k < in; k++) {      // 입력한 값만큼 100개 생산 공정 반복
        makefile(k * MAX_QUEUE_SIZE);          // 반복문의 수 * 100으로 매개 변수에 값을 넘겨줌
